...
#monocle
genemeta <-
  data.frame(gene_short_name = rownames(merge[["RNA"]]@meta.features))
rownames(genemeta) <- rownames(merge@assays$RNA@meta.features)
a <- new_cell_data_set(
  merge@assays$RNA@counts,
...
a <- reduce_dimension(
  ...
  umap.n_neighbors = 20,
  umap.min_dist = 0.1
)#like findclusters, partition is called seurat clusters in seurat
a <- cluster_cells(a)
a <- learn_graph(a)
# prommatical root
#function
get_earliest_principal_node <-
  function(cds,
           time_bin = c("00 d Control", "00 d PLX5622")) {
...
selfroot_cds <-
  order_cells(a, root_pr_nodes = get_earliest_principal_node(a))
setwd("/fs/ess/PCON0022/guoqi/Spinal cord/result/Monocle_prommatical_root")
saveRDS(selfroot_cds,
        "monocleobject_monocleumap_specificparameter_prommaticalroot.rds")
s1 <-
  plot_cells(
    selfroot_cds,
...
s4 <-
  plot_cells(
    selfroot_cds,
    genes = c("P2ry12", "Siglech", "Gpnmb", "Spp1"),
    trajectory_graph_segment_size = 0.4,
    label_leaves = F,
    label_branch_points = F,
    label_cell_groups = F,
...
#density
selfroot_cds_pseudo <-
  selfroot_cds@principal_graph_aux@listData[["UMAP"]][["pseudotime"]]
...
self_data <-
  data.frame(
    row.names = names(selfroot_cds_pseudo),
    pseud = selfroot_cds_pseudo,
    group = rep(0, length(selfroot_cds_pseudo))
  )
